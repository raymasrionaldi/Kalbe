...
class CheckPRFDetail(context: Context, val fm: FragmentManager): Fragment() {
    var ID = 0
    //    val context = this
    ...
    fun konfirmasiCheck(){
        ...
        konfirmasiWin.setMessage("Yakin mau check data ini ?")
            .setPositiveButton("Ya", DialogInterface.OnClickListener{ dialog, which ->
                val check = databaseQueryHelper.updateCheckPRF(ID)
                Toast.makeText(context,"Check $check berhasil", Toast.LENGTH_SHORT).show()
                pindahKeFragmentData()
    ...
    fun pindahKeFragmentData(){
        val fragment = fm.fragments[0] as CheckPRFData
